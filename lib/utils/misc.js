...
const { getWorkspace, getArrayInput, replaceAll, getBoolValue } = github_action_helper_1.Utils;
exports.replaceDirectory = (message) => {
    const workDir = path_1.resolve(getWorkspace());
    return [
        { key: ` -C ${workDir}`, value: '' },
...
exports.isFolding = (title) => !exports.isNoTitle(title) && getBoolValue(core_1.getInput('FOLDING'));
...
exports.homeExpanded = (path) => {
    console.log(path);
    console.log(github_action_helper_1.Utils.getWorkspace());
    console.log(path.indexOf('~') === 0 ? path_1.join(os_1.homedir(), path.substr(1)) : path_1.resolve(github_action_helper_1.Utils.getWorkspace(), path));
    return path.indexOf('~') === 0 ? path_1.join(os_1.homedir(), path.substr(1)) : path_1.resolve(github_action_helper_1.Utils.getWorkspace(), path);
}
exports.cleanPath = (path) => { console.log(os_1.homedir()); return exports.homeExpanded(path).replace(/\s/g, '\\ '); }
