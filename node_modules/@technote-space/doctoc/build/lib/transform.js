...
exports.matchesStart = (checkOpeningComments) => (line) => getTargetComments(checkOpeningComments !== null && checkOpeningComments !== void 0 ? checkOpeningComments : [], __1.CHECK_OPENING_COMMENT).some(comment => new RegExp(comment).test(line));
exports.matchesEnd = (checkClosingComments) => (line) => getTargetComments(checkClosingComments !== null && checkClosingComments !== void 0 ? checkClosingComments : [], __1.CHECK_CLOSING_COMMENT).some(comment => new RegExp(comment).test(line));
...
exports.countHeaders = (headers, mode, moduleName) => {
    const repetitions = {};
...
exports.getLinesToToc = (lines, currentToc, info, processAll) => {
...
exports.getTitle = (title, lines, info) => {
    if (title) {
        return title;
    }
    // eslint-disable-next-line no-magic-numbers
...
exports.transform = (content, { mode, moduleName, maxHeaderLevel, title, isNotitle, isFolding, entryPrefix, processAll, updateOnly, openingComment, closingComment, checkOpeningComments, checkClosingComments, } = {}) => {
