...
const getTitle = (title, lines, info, startSection) => {
    if (title) {
        return title;
    }
    // eslint-disable-next-line no-magic-numbers
    return info.hasStart ? lines[info.startIdx + startSection.length] : __1.DEFAULT_TITLE;
};
...
const determineTitle = (title, isNotitle, isFolding, lines, info, startSection) => {
    if (isNotitle) {
        return '';
    }
    return wrapTitle(exports.getTitle(title, lines, info, startSection), isFolding);
};
...
const transform = (content, options = {}) => {
    ...
    const startSection = params_1.getStartSection(lines, info);
    const extractedOptions = params_1.extractParams(startSection.join(' '));
    const { mode, moduleName, maxHeaderLevel, title, isNotitle, isFolding, entryPrefix, processAll, updateOnly, openingComment, closingComment, isCustomMode, customTemplate, itemTemplate, separator, } = Object.assign(Object.assign({}, options), extractedOptions);
