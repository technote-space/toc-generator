...
var context = require('@actions/github/lib/context');
var githubActionHelper = require('@technote-space/github-action-helper');
var githubActionLogHelper = require('@technote-space/github-action-log-helper');
var filterGithubAction = require('@technote-space/filter-github-action');
...
const { getBranch: getBranch$2 } = githubActionHelper.Utils;
const { isPush: isPush$3 } = githubActionHelper.ContextHelper;
...
const getDate = (index, context) => moment__default["default"]().format(getActionDetail('prDateFormats', context)[index]);
...
const replaceContextVariables = async (string, octokit, context) => githubActionHelper.Utils.replaceVariables(string, await contextVariables(false, octokit, context));
...
const replacePrBodyVariables = async (isComment, prBody, files, output, octokit, context) => githubActionHelper.Utils.replaceVariables(prBody, await prBodyVariables(isComment, files, output, octokit, context));
...
const { getWorkspace: getWorkspace$1, getLocalRefspec, getRefspec } = githubActionHelper.Utils;
const { getOctokit, ensureNotNullValue, useNpm } = githubActionHelper.Utils;
const { getRepository, isPush: isPush$2 } = githubActionHelper.ContextHelper;
const getApiHelper = (octokit, context, logger) => new githubActionHelper.ApiHelper(octokit, context.actionContext, logger);
...
const getGlobalInstallPackagesCommands = (context) => {
    ...
    if (packages.length) {
        if (useNpm(getWorkspace$1(), core.getInput('PACKAGE_MANAGER'))) {
            return [
...
const { getWorkspace, getPrefixRegExp, getAccessToken } = githubActionHelper.Utils;
const { escapeRegExp, replaceAll, getBranch: getBranch$1 } = githubActionHelper.Utils;
const { isPr: isPr$1, isCron, isPush: isPush$1, isCustomEvent, isManualEvent, isWorkflowRun } = githubActionHelper.ContextHelper;
...
const isTargetContext = async (octokit, context) => {
    if (!filterGithubAction.isTargetEvent(context.actionDetail.targetEvents ?? DEFAULT_TARGET_EVENTS, context.actionContext)) {
        return false;
    }
    ...
    return filterGithubAction.isTargetLabels(toArray(getActionDetail('includeLabels', context, () => [])), [], context.actionContext);
};
...
const getHelper = (context) => new githubActionHelper.GitHelper(new githubActionLogHelper.Logger(replaceDirectory), {
    depth: -1,
...
const isSetApiToken = () => !!core.getInput('API_TOKEN');
const getApiToken = () => core.getInput('API_TOKEN') || getAccessToken(true);
...
const { sleep, getBranch, objectGet } = githubActionHelper.Utils;
const { isPr, isPush } = githubActionHelper.ContextHelper;
const commonLogger = new githubActionLogHelper.Logger(replaceDirectory);
...
const outputResults = (results) => {
    ...
    core.setOutput('result', getOutputResult(results));
};
const runCreatePr = async (isClose, getPulls, octokit, context) => {
    const logger = new githubActionLogHelper.Logger(replaceDirectory, true);
    const results = [];
    const processed = {};
...
const { showActionInfo } = githubActionHelper.ContextHelper;
const getLogger = (logger) => logger ?? new githubActionLogHelper.Logger();
/* istanbul ignore next */
const getContext = (option) => option.context ?? new context.Context();
...
async function main(option) {
    ...
    const octokit = githubActionHelper.Utils.getOctokit();
