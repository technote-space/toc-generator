...
exports.getCommitName = (context) => exports.getActionDetail('commitName', context);
exports.getCommitEmail = (context) => exports.getActionDetail('commitEmail', context);
...
const contextVariables = (context) => {
    ...
    return [
        { key: 'PR_NUMBER', replace: () => getPrParam(pr => pr.number) },
        { key: 'PR_NUMBER_REF', replace: () => getPrParam(pr => pr.number ? `#${pr.number}` : getDefaultBranchUrl(context)) },
        { key: 'PR_ID', replace: () => getPrParam(pr => pr.id) },
        { key: 'PR_HEAD_REF', replace: () => getPrParam(pr => pr.head.ref) },
        { key: 'PR_BASE_REF', replace: () => getPrParam(pr => pr.base.ref) },
        { key: 'PR_TITLE', replace: () => getPrParam(pr => pr.title) },
        { key: 'PR_URL', replace: () => getPrParam(pr => pr.html_url) },
        ...
        key: `VARIABLE${index + 1}`, replace: () => getVariable(index, context),
        ...
        // eslint-disable-next-line no-magic-numbers
        key: `DATE${index + 1}`, replace: () => getDate(index, context),
    })));
};
/**
 ...
 */
const replaceVariables = (string, variables) => variables.reduce((acc, value) => replaceAll(acc, `\${${value.key}}`, value.replace()), string);
/**
 * @param {string} string string
 * @param {ActionDetails} context action details
 * @return {string} replaced
 */
const replaceContextVariables = (string, context) => replaceVariables(string, contextVariables(context));
...
exports.isActionPr = (context) => getPrefixRegExp(getPrBranchPrefix(context)).test(exports.getPrHeadRef(context));
exports.getPrBranchName = (context) => isPush(context.actionContext) ? getBranch(context.actionContext) : (getPrBranchPrefix(context) + replaceContextVariables(exports.getActionDetail('prBranchName', context), context));
exports.getPrTitle = (context) => replaceContextVariables(exports.getActionDetail('prTitle', context), context);
...
const prBodyVariables = (files, output, context) => {
    const toCode = (string) => string.length ? ['', '```Shell', string, '```', ''].join('\n') : '';
    return [
        {
            key: 'PR_LINK',
            replace: () => exports.getPrLink(context),
        },
        {
            key: 'COMMANDS',
        ...
        },
        {
            key: 'FILES_SUMMARY',
            // eslint-disable-next-line no-magic-numbers
            replace: () => 'Changed ' + (files.length > 1 ? `${files.length} files` : 'file'),
        },
        {
            key: 'ACTION_NAME',
            replace: () => context.actionDetail.actionName,
        },
        {
            key: 'ACTION_OWNER',
            replace: () => context.actionDetail.actionOwner,
        },
        {
            key: 'ACTION_REPO',
            replace: () => context.actionDetail.actionRepo,
        },
        {
            key: 'ACTION_URL',
            replace: () => `https://github.com/${context.actionDetail.actionOwner}/${context.actionDetail.actionRepo}`,
        },
        {
            key: 'ACTION_MARKETPLACE_URL',
            replace: () => `https://github.com/marketplace/actions/${context.actionDetail.actionRepo}`,
        },
    ].concat(contextVariables(context));
};
const replacePrBodyVariables = (prBody, files, output, context) => replaceVariables(prBody, prBodyVariables(files, output, context));
exports.getPrBody = (files, output, context) => replacePrBodyVariables(exports.getActionDetail('prBody', context).trim().split(/\r?\n/).map(line => line.replace(/^[\s\t]+/, '')).join('\n'), files, output, context);
...
exports.isTargetContext = (context) => {
    ...
    if (exports.isActionPr(context) || !exports.isTargetBranch(exports.getPrHeadRef(context), context)) {
        return false;
    }
