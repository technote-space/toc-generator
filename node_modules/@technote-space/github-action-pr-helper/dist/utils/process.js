...
const checkActionPr = (logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    const defaultBranch = yield command_1.getDefaultBranch(octokit, context);
    if (defaultBranch === misc_1.getPrHeadRef(context)) {
        return true;
    }
...
const createCommit = (addComment, isClose, logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { files, output, aborted } = yield command_1.getChangedFiles(helper, logger, octokit, context);
    if (!files.length) {
        logger.info('There is no diff1.');
        logger.info(context.isBatchProcess);
        if (context.isBatchProcess) {
            ...
            if (pr && aborted) {
                ...
                return getResult('succeeded', yield command_1.resolveConflicts(branchName, helper, logger, octokit, context), context);
            }
        }
...
const createPr = (makeGroup, isClose, helper, logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!misc_1.isActionPr(context) && !(yield misc_1.isTargetBranch(misc_1.getPrHeadRef(context), octokit, context))) {
        return getResult('skipped', 'This is not a target branch', context);
    }
    
    console.log(context.actionContext.payload.pull_request);
    console.log(misc_1.isActionPr(context));
    if (misc_1.isActionPr(context) || misc_1.isNotCreatePR(context)) {
        ...
        return createCommit(misc_1.isActionPr(context), isClose, logger, octokit, context);
    }
    ...
    if (!mergeable) {
        // Resolve conflicts if PR is not mergeable
        detail = yield command_1.resolveConflicts(branchName, helper, logger, octokit, context);
    }
...
const outputResults = (results) => {
    ...
    core_1.setOutput('result', getOutputResult(results));
};
const runCreatePr = (isClose, getPulls, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        for (var _b = __asyncValues(getPulls(octokit, context)), _c; _c = yield _b.next(), !_c.done;) {
            ...
            const isTarget = misc_1.isActionPr(actionContext) || (yield misc_1.isTargetBranch(misc_1.getPrHeadRef(actionContext), octokit, actionContext));
            let target = '';
            if (isTarget) {
                target = yield variables_1.getPrBranchName(helper, octokit, actionContext, true);
