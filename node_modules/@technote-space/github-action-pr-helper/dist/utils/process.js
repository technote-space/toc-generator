...
const checkActionPr = (logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    const pr = yield command_1.findPR(misc_1.getPrHeadRef(context), logger, octokit, context);
    if (!pr) {
    ...
    const basePr = yield command_1.findPR(pr.base.ref, logger, octokit, context);
    if (!basePr) {
...
const createCommit = (addComment, logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    const helper = misc_1.getHelper(context);
    const branchName = yield variables_1.getPrBranchName(helper, logger, octokit, context);
    if (!(yield misc_1.isTargetBranch(branchName, octokit, context))) {
    ...
    if (!files.length) {
        ...
        if (context.isBatchProcess) {
            const pr = yield command_1.findPR(branchName, logger, octokit, context);
...
const createPr = (makeGroup, isClose, logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    if (makeGroup) {
...
const runCreatePr = (isClose, getPulls, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        for (var _b = __asyncValues(getPulls(octokit, context)), _c; _c = yield _b.next(), !_c.done;) {
            const pull = _c.value;
            try {
                results.push(yield createPr(true, isClose, logger, octokit, misc_1.getActionContext(context, pull)));
            }
            catch (error) {
                results.push(getResult('failed', error.message, misc_1.getActionContext(context, pull)));
            }
...
function pullsForSchedule(octokit, context) {
    return __asyncGenerator(this, arguments, function* pullsForSchedule_1() {
        ...
        const processed = {};
        try {
            for (var _b = __asyncValues(command_1.getApiHelper(logger).pullsList({}, octokit, context.actionContext)), _c; _c = yield __await(_b.next()), !_c.done;) {
                const pull = _c.value;
                if (pull.base.ref in processed) {
                    continue;
                }
                processed[pull.base.ref] = true;
                yield yield __await(pull);
            }
        }
        ...
        if (misc_1.checkDefaultBranch(context) && !((yield __await(command_1.getDefaultBranch(octokit, context))) in processed)) {
            yield yield __await(yield __await(misc_1.getPullsArgsForDefaultBranch(octokit, context)));
        }
    });
}
const runCreatePrAll = (octokit, context) => __awaiter(void 0, void 0, void 0, function* () { return runCreatePr(false, pullsForSchedule, octokit, context); });
/**
 * @param {GitHub} octokit octokit
 * @param {Context} context context
 * @return {AsyncIterable} pull
...
const runCreatePrClosed = (octokit, context) => __awaiter(void 0, void 0, void 0, function* () { return runCreatePr(true, pullsForClosed, octokit, context); });
exports.execute = (octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    if (misc_1.isClosePR(context)) {
        ...
        yield createCommit(false, commonLogger, octokit, context);
    }
    else if (isPr(context.actionContext)) {
        yield outputResult(yield createPr(false, false, commonLogger, octokit, context), true);
    }
    else {
