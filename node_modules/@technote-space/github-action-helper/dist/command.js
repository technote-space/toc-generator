...
class Command {
    ...
    constructor(logger) {
         ...
         * @return {void} void
         */
        this.execCallback = (command, altCommand, quiet, suppressOutput, resolve, reject) => (error, stdout, stderr) => {
            if (error) {
        ...
        this.execAsync = (args) => new Promise((resolve, reject) => {
            ...
            if (typeof cwd === 'undefined') {
                child_process_1.exec(this.getCommand(command, quiet, suppressError), this.execCallback(command, altCommand, quiet, suppressOutput, resolve, reject));
            }
            else {
                child_process_1.exec(this.getCommand(command, quiet, suppressError), { cwd }, this.execCallback(command, altCommand, quiet, suppressOutput, resolve, reject));
            }
        });
    }
}
