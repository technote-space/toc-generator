...
class ApiHelper {
    ...
    constructor(logger, options) {
         ...
         */
        this.pullsList = (branchName, octokit, context, state = 'open') => __awaiter(this, void 0, void 0, function* () {
        ...
        this.createPR = (rootDir, commitMessage, files, createBranchName, detail, octokit, context) => __awaiter(this, void 0, void 0, function* () {
            ...
            const pulls = yield this.pullsList(branchName, octokit, context, 'all');
            if (pulls.data.length) {
                if (pulls.data[0].state === 'closed') {
				    this.logger.startProcess('Reopening PullRequest... [%s]', branchName);
				    yield this.pullsUpdate(pulls.data[0].number, {
					    state: 'open',
					    base: undefined,
				    }, octokit, context);
                }
                this.logger.startProcess('Updating PullRequest... [%s] -> [%s]', branchName, yield this.getRefForUpdate(false, octokit, context));
                const updated = yield this.pullsUpdate(pulls.data[0].number, detail, octokit, context);
                this.logger.endProcess();
                return updated.data;
            }
            else {
                this.logger.startProcess('Creating PullRequest... [%s] -> [%s]', branchName, yield this.getRefForUpdate(false, octokit, context));
                const created = yield this.pullsCreate(branchName, detail, octokit, context);
                this.logger.endProcess();
                return created.data;
            }
        });
